...
HBAR = 1.0
MASS = 1.0
G_COEFF = 0.12        # Balanced lower to prevent self-repulsion from breaking the latch
RHO_MAX = 1.5
DX = 1.0
DT = 0.04
...
# =====================================================================
# 3. COMPONENT INITIALIZATION & THE TOROIDAL LATCH FIELD
# =====================================================================
...
# Define the exact 6 scanner contour nodes enclosing the origin
latch_ring = [(1, -1, 0), (0, -1, 1), (-1, 0, 1), (-1, 1, 0), (0, 1, -1), (1, 0, -1)]

for idx, (q, r, s) in enumerate(nodes):
    dist_global = np.sqrt(q**2 + r**2 + s**2)
    v_ext[idx] = 0.5 * 12.0 * (dist_global / RADIUS)**4

    # BREAKTHROUGH UPGRADE: Establish the Toroidal (Donut) Potential Well
    if (q, r, s) == (0, 0, 0):
        v_ext[idx] = 10.0   # Central repulsive barrier (The Pin/Island)
    elif (q, r, s) in latch_ring:
        v_ext[idx] = -30.0  # Deep circular attractive well (The Ring Racetrack)
    ...
    # Vortex A (Clockwise Spin, Moving Right)
    if dist_vortex_A < 1.5:
        rho = RHO_MAX
        vx = -(r - r_left) * 0.4 + 0.5
        vy = (q - q_left) * 0.4

    # Vortex B (MATCHED Clockwise Chirality, Moving Left)
...
# =====================================================================
print("\n[ TOROIDAL LATCH ENGAGED ] EXECUTING ENTRAPMENT IN RING METHOD")
print(f"Lattice Footprint: {num_nodes} Nodes | Ring Latch Wells: -30.0 Locked")
print("=" * 85)
time.sleep(1)
...
# =====================================================================
# 5. LINE INTEGRAL SCANNER OVER THE RACETRACK WELLS
# =====================================================================
print("-" * 85)
print("[COMPILER RUNTIME] ENGAGING TOPO-SCANNER OVER TOROIDAL RACETRACK...")

contour_indices = [coord_to_idx[c] for c in latch_ring]
total_phase = 0.0
...
if w_charge >= 1:
    print(f"[COMPILER OUTPUT] >>> LOGICAL STATE: {w_charge} (TRUE / LATCH SUCCESSFUL INSIDE TOROIDAL MEMORY CELL)")
elif w_charge == 0:
    print("[COMPILER OUTPUT] >>> LOGICAL STATE: 0 (FALSE / VACANT RACETRACK)")
else:
